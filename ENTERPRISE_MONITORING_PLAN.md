...
## ফেজ C — স্তর ৩: রিলিজ-ক্যানারি (একদম নতুন, ৩ ধাপ) — কোড লেখা হয়েছে, sandbox-এ যাচাই-অসম্পূর্ণ (ফেজ B-এর প্যাটার্নের মতোই)

**⚠️ যাচাই-অবস্থা (২২ জুলাই ২০২৬):** কোড sandbox-এ লেখা হয়েছে এবং `node --check`
দিয়ে সিনট্যাক্স যাচাই করা হয়েছে, `.github/workflows/build-apk.yml`-এর YAML
`python3 -c "import yaml..."` দিয়ে পার্স করে valid নিশ্চিত করা হয়েছে —
কিন্তু ফেজ B-এর মতোই এই sandbox-এ real Firestore Emulator চালানো যায়নি
(network egress allowlist-এ `storage.googleapis.com` নেই)। তাই বক্স `[x]`
করা হলো **না** — GitHub-এ push করার পর real CI রান দেখেই টিক দিতে হবে (ফেজ
B-তে যেমন Build #408 পর্যন্ত অপেক্ষা করা হয়েছিল, একই নিয়ম এখানেও প্রযোজ্য)।

- [ ] **C1.** End-to-end canary স্ক্রিপ্ট লেখা: emulator-এ ইনভয়েস তৈরি → সিঙ্ক → ব্যাকআপ → রিস্টোর → ভয়েড — প্রতি ধাপ automatic ভ্যালিডেশনসহ — **কোড লেখা হয়েছে:** `tests/release-canary.mjs` (নতুন), sequential/dependency-chained ৫-ধাপ (`tests/sync-emulator-tests.mjs`-এর মতো independent case-by-case না) — প্রতিটা ধাপ `runStep()` দিয়ে wrap করা, নিজে থেকেই assert করে (status/total ঠিক আছে কিনা, changed=true, content-hash অ-শূন্য, zero-drift, rules-guard কাজ করছে কিনা ইত্যাদি)
- [ ] **C2.** এটাকে release workflow-এ গেট হিসেবে যোগ করা (fail হলে release/APK build আটকাবে) — `package.json`-এ নতুন script `test:release-canary`, `.github/workflows/build-apk.yml`-এর `firestore-rules` জবে (যেটার উপর `build` জব `needs:` নির্ভরশীল) নতুন blocking step হিসেবে যোগ করা হয়েছে B4-এর ঠিক পরে
- [ ] **C3.** ক্যানারি ফেইলের ক্ষেত্রে স্পষ্ট রিপোর্ট (কোন ধাপে fail করলো) তৈরি করা — `printReport()` ফাংশন প্রতিটা ধাপের ✅/❌ তালিকা প্রিন্ট করে, প্রথম যে ধাপে fail করেছে সেটা আলাদাভাবে হাইলাইট করে এবং বাকি ধাপ কেন স্কিপ হলো তা ব্যাখ্যা করে (dependency-chained হওয়ায় corrupted অবস্থার উপর পরের ধাপ চালানো হয় না)

**নোট:** `package.json`-এর নতুন script লাইনটা GitHub-এ upload করার সময়
ফেজ B-তে যেমন বাদ পড়ে গিয়েছিল (Build #405/#406-এর মূল কারণ), এই ফেজেও
সেই একই ভুল এড়াতে "Add file via upload"-এ পুরো `package.json` ফাইল ঠিকভাবে
আপলোড হয়েছে কিনা re-upload-এর সময় ডাবল-চেক করতে হবে।
